...
while coffee_machine_on is True:
    ...
    if coffee_choice == "off": # Creating an option to turn the machine off, this will be a 'secret' response known by a maitenance worker.
        ...
        drink = menu[coffee_choice] # Depending on which drink the user chooses, the ingredients and cost will be printed.
        if sufficient_resource(drink["ingredients"]): # Calling my function. under menu, it will fetch the ingredients and print them out
            payment = process_coins() # Calling my function, capturing user payment in my variable 'payment'
